...
public class Plotter implements ContextInterface {
	...
	// and the maximum stroke width for the lines which represent the weights of the
	// NN
	public void plot(NeuralNetwork nn, int nodeSize, int spaceX, int spaceY, int maxStrokeWidth) {
		...
		// Creating the window which contains the GraphicsEngine to display our
		// NeuralNetwork
		Window w = new Window("NeuralNetwork", width, height);
		cp = new ContextPane(w, this);
	...
	public void drawRev(Graphics2D g) {
		...
		if (setInput) {
			// Since the coordinate system is flipped we have to reverse it to display the
			// text upright
		...
		for (int l = 0; l < nn.layer.size(); l++) {
			for (int p = 0; p < nn.layer.get(l).pcts.size(); p++) {
				...
				int height = (nn.layer.get(l).pcts.size() * nodeSize) + ((nn.layer.get(l).pcts.size() - 1) * spaceY)
						+ 40;
		...
		for (int l = 0; l < nn.layer.size() - 1; l++) {
			for (int p = 0; p < nn.layer.get(l).pcts.size(); p++) {
				for (int np = 0; np < nn.layer.get(l + 1).pcts.size(); np++) {
					...
					// Absolute value since .setStroke takes only positive values
					...
					g.drawLine(posX[l][p] + offsToMiddle, posY[l][p] + offsToMiddle, posX[l + 1][np] + offsToMiddle,
							posY[l + 1][np] + offsToMiddle);
				}
			}
		}
	...
	@Override
	public void loop(ContextPane cp) {
